...
class LLMClient:
    ...
    def chat_json(
        ...
        return self.parse_json_output(response.answer, output_schema)
    ...
    @staticmethod
    def parse_json_output(output_text: str, output_schema: Type[T]) -> T:
        try:
            data = json.loads(output_text)
        except json.JSONDecodeError as e:
            raise ValueError(f"模型输出不是合法 JSON: {output_text}") from e

        try:
            return output_schema.model_validate(data)
        except Exception as e:
            raise ValueError(f"模型输出 JSON 不符合 schema: {data}") from e
